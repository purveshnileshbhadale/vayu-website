...
def _groq_client():
    try:
        from groq_client import client as gclient
        return gclient
    except Exception:
        return None


def _try_providers(prompt: str, system: str, max_tokens: int = 200, temperature: float = 0.0) -> str:
    or_c = _or_client()
    if or_c:
        try:
            return or_c.chat(prompt, system=system, max_tokens=max_tokens, temperature=temperature)
        except Exception as e:
            print(f"[Memory] ⚠️ OpenRouter failed: {e}")

    gr_c = _groq_client()
    if gr_c:
        try:
            return gr_c.chat(prompt, system=system, max_tokens=max_tokens, temperature=temperature)
        except Exception as e:
            print(f"[Memory] ⚠️ Groq failed: {e}")

    raise RuntimeError("Both OpenRouter and Groq unavailable for memory operations.")


def _try_providers_json(prompt: str, system: str, max_tokens: int = 500) -> dict:
    or_c = _or_client()
    if or_c:
        try:
            return or_c.chat_json(prompt, system=system, max_tokens=max_tokens)
        except Exception as e:
            print(f"[Memory] ⚠️ OpenRouter JSON failed: {e}")

    gr_c = _groq_client()
    if gr_c:
        try:
            return gr_c.chat_json(prompt, system=system, max_tokens=max_tokens)
        except Exception as e:
            print(f"[Memory] ⚠️ Groq JSON failed: {e}")

    raise RuntimeError("Both OpenRouter and Groq unavailable for JSON memory operations.")


def should_extract_memory(user_text: str, vayu_text: str, api_key: str = "") -> bool:
    try:
...
def extract_memory(user_text: str, vayu_text: str, api_key: str = "") -> dict:
    try:
        combined = f"User: {user_text[:600]}\nVayu: {vayu_text[:300]}"

        result = _try_providers_json(
            ...
            f"                  (e.g. vayu_core: 'Building a VAYU-like AI assistant')\n"
            ...
            f"- Extract from BOTH user and Vayu turns.\n"
            ...
            f' "projects":{{"vayu_core":{{"value":"VAYU-like AI assistant"}}}},\n'
