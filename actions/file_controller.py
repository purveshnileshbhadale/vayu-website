...
def read_file(path: str, max_chars: int = 3000) -> str:
    """Reads and returns the content of any file type (txt, docx, pdf, html, etc.)."""
    try:
        target = Path(path).expanduser()
        if not target.exists():
        ...
        text_exts = {".txt", ".md", ".rst", ".log", ".csv", ".json", ".xml",
                     ...
                     ".css", ".java", ".c", ".cpp", ".h", ".hpp",
                     ".sh", ".bat", ".ps1", ".sql", ".rb", ".php", ".go",
                     ".rs", ".swift", ".kt", ".lua", ".r", ".pl", ".m",
        ...
        if ext == ".pdf":
            try:
                ...
                text = f"[PDF file: {target.name}] Install pdfplumber to read content."
        ...
        if ext == ".docx":
            try:
                ...
                return f"[Word doc: {target.name}] Install python-docx to read content."
            except Exception as e:
                return f"[Word doc] Read failed: {e}"
        ...
        if ext in (".pptx", ".ppt"):
            try:
                ...
                return f"[PPT: {target.name}] Install python-pptx to read content."
            except Exception as e:
                return f"[PPT] Read failed: {e}"
        ...
        if ext == ".epub":
            try:
                ...
                book = epub.read_epub(str(target))
                chapters = []
                ...
                full = '\n\n'.join(chapters)
                return full[:max_chars] + (f"\n\n... (truncated, {len(full)} total chars)" if len(full) > max_chars else "")
            except ImportError:
                return f"[EPUB: {target.name}] Install ebooklib to read content."
        ...
        if ext in (".rtf",):
            try:
                ...
                return f"[RTF: {target.name}] Install striprtf to read content."
        ...
        if ext in (".xlsx", ".xls", ".ods"):
            try:
                ...
                return f"[Spreadsheet: {target.name}] Install pandas openpyxl to read content."

        return f"File: {target.name} ({_format_size(target.stat().st_size)}). Text preview not available for this format."
